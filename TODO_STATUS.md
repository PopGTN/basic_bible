...
## Completed Recently
- `done` Removed the extra introduction description line that was being repeated from TOC labels, and added a persisted `Show Chapter Headers` reader setting so the large book/chapter header can be disabled from settings or the reader sheet when the source document does not repeat that structure.
